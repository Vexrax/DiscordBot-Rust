...
pub async fn run(_options: &[ResolvedOption<'_>], ctx: &Context, command: &CommandInteraction) {
    ...
    let match_embed = build_embed(game_name.to_string(), tagline.to_string(), match_players);
    let _ = command.channel_id.send_message(&ctx.http,CreateMessage::new().tts(false).embed(match_embed)).await;
}

fn build_embed(main_player_game_name: String, main_player_tagline: String, match_players: Vec<MatchPlayer>) -> CreateEmbed {
    ...
    for match_player in match_players {
        ...
        fields.push((title, display_rank, false));
    }
